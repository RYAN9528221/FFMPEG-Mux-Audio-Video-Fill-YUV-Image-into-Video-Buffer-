...
#define STREAM_DURATION   3//大約10秒
...
typedef struct OutputStream {
	...
	struct SwsContext* sws_ctx2;//RGB2YUV
...
int main(int argc, char** argv)
{
	...
	const char* out_filename = "hello.mp4";//輸出檔案
	...
	if ((ret = avformat_open_input(&video_ctx, "quanta.mp4", NULL, NULL)) < 0)
	{
...
//試試看解碼
	...
	if (out_fmt->video_codec != AV_CODEC_ID_NONE)
	{
		...
		//video_st.st->time_base = AVRational{ 1, STREAM_FRAME_RATE };
	...
	if (out_fmt->audio_codec != AV_CODEC_ID_NONE) 
	{
		...
		//用來告訴輸出端的輸入流
		AVStream* in_stream;
		in_stream = video_ctx->streams[audio_index];//聲音部分
		AVCodecContext* dec_ctx = in_stream->codec;
		//=========================================
